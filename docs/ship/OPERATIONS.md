...
| "The same day appears twice in my journal" / totals look one too high | Two completions for one civil day, from a build before the completion guard failed closed | Ask them to force-quit and reopen | `CompletionReconciler` merges them on the next launch, keeping **every** reflection written against that day joined by a blank line. A badge already earned on the inflated count is not revoked. If it recurs after a relaunch, the guard has regressed — that is a P1 |
| "Two of my reflections got joined together" | The repair above, working as designed | Which date | Nothing was lost: both texts are theirs, for that day. They can edit the merged entry freely. If it is over the 2,000-character limit the editor asks them to trim before saving |
