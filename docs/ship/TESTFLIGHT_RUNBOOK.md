# TestFlight Runbook — for the owner
...
> **Hard rule:** Scout/agents must not create App Store Connect records, upload builds, or submit anything. Every step below is the owner's to run. Internal TestFlight is fine when the owner chooses; **external/public TestFlight and App Store submission are blocked** until the blockers at the bottom are cleared.
