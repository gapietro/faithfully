...
| Internal TestFlight (#27) | 🟡 Greg’s clicks — runbook ready |
| Pre-public gate (#28) | 🟡 Explicit Greg OK after green lights |
| Epic G beta polish (#21, #29–#33) | ⏳ After first internal TF preferred |

**Traffic light board:** [`docs/ship/README.md`](README.md)

**Bottom line:** Internal TestFlight is unblocked on content/privacy/icon once Greg does signing (#27). **Public** release still needs good screenshots (#26), then #28. Badge art is Epic G #32, not launch-hard-blocked.
...
|---|--------|-----------|--------|
| **#26** | Final app icon + App Store screenshots | Scout / Greg | **Icon done.** Screenshots remaining. |
| **#27** | Internal TestFlight — Greg signing & first build | **Greg** | [`TESTFLIGHT_RUNBOOK.md`](TESTFLIGHT_RUNBOOK.md) |
| **#28** | Pre-public gate | **Greg OK** | Do not coerce agents to Submit |
...
1. **Scripture v1 = public domain only** (Greg, #22)  
...
3. **Agents must not** create ASC records, upload TF builds, or Submit for Review without explicit Greg OK on Telegram.

4. **Recraft** is approved for brand art (icon done; badges = #32). Key is **not in git**.

---

## Secrets & tooling (Mac mini)

| Item | Where |
|------|--------|
| Recraft API key | macOS Keychain service **`recraft.ai`**, account `scout.a.pietro` — API validated HTTP 200 against `external.api.recraft.ai` |
...
| `docs/ship/TESTFLIGHT_RUNBOOK.md` | Greg TF steps; blockers list |
...
2. **Greg #27** internal TestFlight using runbook  
3. Optional parallel: **#32** Recraft badge pack (same Keychain key)  
4. **#28** only after Greg explicit OK  

---

## Contact / trust

- Greg Pietro — Telegram primary command channel  
- Support email used in privacy: `gapietro@gmail.com`  

Do not take store-shipping actions without Greg on Telegram.
