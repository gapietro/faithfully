...
One-page status for the owner. Detail lives in the linked docs.
...
**Bottom line:** the app is technically ready for an internal TestFlight build today (the owner's signing steps only). Public release is blocked on exactly two red items — scripture text accuracy and translation licensing — plus the mechanical yellows (privacy URL, icon, screenshots).
